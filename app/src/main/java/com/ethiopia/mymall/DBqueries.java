...
public class DBqueries {
                        ...
                        if (task.isSuccessful()){
                            for (QueryDocumentSnapshot documentSnapshot : task.getResult()){
                                ...
                                if ((long)documentSnapshot.get("view_type") == 0){
                                    ...
                                    List<HorizontalProductScrollModel> GridLayoutModeList = new ArrayList<>();
                                    long no_of_products = (long) documentSnapshot.get("no_of_products");
                                    for (long x = 1;x < no_of_products + 1;x++){
                                        GridLayoutModeList.add(new HorizontalProductScrollModel(documentSnapshot.get("product_ID_"+x).toString()
                                                ,documentSnapshot.get("product_image_"+x).toString()
                                                ,documentSnapshot.get("product_title_"+x).toString()
                                                ,documentSnapshot.get("product_subtitle_"+x).toString()
                                                ,documentSnapshot.get("product_price_"+x).toString()));
                    ...
                    for (long x = 0;x < (long)task.getResult().get("list_size");x++){
                        ...
                        if (DBqueries.wishList.contains(ProductDetailsActivity.productID)){
                            ...
                            if (ProductDetailsActivity.addToWishlistBtn != null) {
                                ProductDetailsActivity.addToWishlistBtn.setSupportBackgroundTintList(context.getResources().getColorStateList(R.color.colorPrimary));
                            }
                        }else {
                            if (DBqueries.wishList.contains(ProductDetailsActivity.productID)) {
    ...
    public static void removedFromWishlist(final int index, final Context context){
